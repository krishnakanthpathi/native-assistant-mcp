...
def register(mcp: FastMCP):
    ...
    def move_window(window_id: int, x: int, y: int) -> str:
        """Move a window to (x, y) screen coordinates via Accessibility API."""
        swift_code = '''
import Cocoa
        ...
        if targetTitle != nil && title == targetTitle {
            ...
            if let axPos = AXValueCreate(.cfPoint, &pos) {
                ...
                if err == .success {
                    ...
                    print("Accessibility error setting position: " + String(err.rawValue))
                    exit(1)
                }
            }
        }
...
exit(1)
'''
        res = subprocess.run(['swift', '-', str(window_id), str(x), str(y)], input=swift_code, capture_output=True, text=True)
        return res.stdout.strip() or 'Window moved.'
